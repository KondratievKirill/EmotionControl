...
def add_data(data, emotion, type_media):
    ...
    cur = con.cursor()
    emotion_id = cur.execute('''SELECT emotion_id FROM Emotions WHERE emotion_name=?''', (emotion,)).fetchall()
...
def get_data(type_media, emotion_name):
    ...
    WHERE emotion_id IN (SELECT emotion_id from Emotions WHERE emotion_name=?) AND 
